...
async function search() {
  ...
  try {
    results.value = await apiFetch<any[]>(`/api/admin/bestbuy/search?q=${encodeURIComponent(query.value.trim())}`)
  } catch (e: any) {
    error.value = e.data?.message ?? 'Search failed. Check your Best Buy API key.'
  } finally {
...
const toggleAll = () => {
  selected.value = selected.value.size === results.value.length
    ? new Set()
    : new Set(results.value.map(r => r.bestBuySkuId))
}

async function importSelected() {
  const items = results.value
    .filter(r => selected.value.has(r.bestBuySkuId))
  ...
  error.value = ''
  try {
    const res = await apiFetch<any>('/api/admin/bestbuy/import', { method: 'POST', body: { items } })
...
<template>
  <div class="p-8 max-w-5xl mx-auto">
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-900">Best Buy Product Import</h1>
      <p class="text-sm text-gray-500 mt-1">Search Best Buy and import products with your CJ affiliate tag embedded automatically.</p>
    </div>

    <form class="flex gap-3 mb-4" @submit.prevent="search">
      <input
        v-model="query"
        placeholder="Search Best Buy (e.g. 4K TV)"
    ...
    <div v-if="results.length">
      ...
      <div class="space-y-2">
        <div
          ...
          :key="item.bestBuySkuId"
          :class="selected.has(item.bestBuySkuId) ? 'ring-2 ring-primary-500 bg-primary-50' : 'bg-white hover:bg-gray-50'"
          class="flex items-center gap-4 p-4 rounded-xl border border-gray-200 cursor-pointer transition-all"
          @click="toggle(item.bestBuySkuId)"
        >
          <input type="checkbox" :checked="selected.has(item.bestBuySkuId)" class="rounded flex-shrink-0" @click.stop="toggle(item.bestBuySkuId)" />
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-gray-400 mt-0.5">SKU: {{ item.bestBuySkuId }}</p>
          </div>
    ...
    <p v-else-if="!searching" class="text-gray-400 text-sm text-center py-16">
      Search Best Buy to find products to import.
    </p>
  </div>
</template>
